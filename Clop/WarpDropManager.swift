...
import Foundation
import os
// Ziben custom: WarpDrop is a private local-only package (drop.lowtechguys.com).
// It is unavailable outside the upstream author's machine, so the framework
// import and its client calls are stubbed out. The session/manager types are
// kept so all call sites keep compiling; sharing is simply a no-op.
...
private func warpDropSendFiles(_ files: [URL], overlayOptimiser: Optimiser?) {
    // Ziben custom: WarpDrop sharing disabled (private package unavailable).
    overlayOptimiser?.warpDropConnecting = false
    overlayOptimiser?.overlayMessage = "WarpDrop disabled"
    log.info("WarpDrop send requested but disabled in this build")
}

/// Send a single file securely and await the share link.
/// Returns the share URL on success, nil on failure or timeout.
@MainActor
func warpDropSendAndWait(url _: URL, optimiser: Optimiser) async -> String? {
    // Ziben custom: WarpDrop sharing disabled (private package unavailable).
